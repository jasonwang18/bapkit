...
public class CustomTagViewActivity extends AppCompatActivity {

    private CustomFlowTagView mColorFlowTagLayout;
    private CustomFlowTagView mSizeFlowTagLayout;
    private CustomFlowTagView mMobileFlowTagLayout;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ac_custom_tag_view);

        mColorFlowTagLayout = (CustomFlowTagView) findViewById(R.id.color_flow_layout);
        mSizeFlowTagLayout = (CustomFlowTagView) findViewById(R.id.size_flow_layout);
        mMobileFlowTagLayout = (CustomFlowTagView) findViewById(R.id.mobile_flow_layout);

        //颜色
        mColorTagAdapter = new TagAdapter<>(this);
        mColorFlowTagLayout.setAdapter(mColorTagAdapter);
        mColorFlowTagLayout.setOnTagClickListener(new CustomFlowTagView.OnTagClickListener() {
            @Override
            public void onItemClick(CustomFlowTagView parent, View view, int position) {
        ...
        mSizeFlowTagLayout.setTagCheckedMode(CustomFlowTagView.FLOW_TAG_CHECKED_SINGLE);
        mSizeFlowTagLayout.setAdapter(mSizeTagAdapter);
        mSizeFlowTagLayout.setOnTagSelectListener(new CustomFlowTagView.OnTagSelectListener() {
            @Override
            public void onItemSelect(CustomFlowTagView parent, List<Integer> selectedList) {
        ...
        mMobileFlowTagLayout.setTagCheckedMode(CustomFlowTagView.FLOW_TAG_CHECKED_MULTI);
        mMobileFlowTagLayout.setAdapter(mMobileTagAdapter);
        mMobileFlowTagLayout.setOnTagSelectListener(new CustomFlowTagView.OnTagSelectListener() {
            @Override
            public void onItemSelect(CustomFlowTagView parent, List<Integer> selectedList) {
                if (selectedList != null && selectedList.size() > 0) {
                    StringBuilder sb = new StringBuilder();
